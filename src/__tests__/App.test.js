...
describe('<App /> integration', () => {
  ...
  test("check if number of events is properly passed on as a prop to NumberOfEvents", () => {
    ...
    expect(AppWrapper.find(NumberOfEvents).props().noe).toBe(20);
    AppWrapper.unmount();
  });
  ...
  test("check if state in the app changes on input change in NumberOfEvents", () => {
    ...
    NumberOfEventsWrapper.find(".noe-input").simulate("change", {
      target: { value: 12 },
    });
    expect(NumberOfEventsWrapper.state("noe")).toBe(12);
